...
function divandjog(mask,pmn,xi,x,f,Labs,epsilon2,csteps,lmask,pcmethod=1; alphapc=[],otherargs...
	...
	diagshift=0.0000001;	
			...
			function compPC4ter(iB,H,R)
				return x -> diagshift*x+PC2*x-scalef2*(PC2*(PC1*(PC2*x)))
			end
...
######
		...
		if methodpccoarse==3
			...
			function compPC3(iB,H,R)
				...
				return x -> diagshift*x+scalef2*(HI*(PC1*(PC2*(PC1*(HI'*x)))))
			end
		...
		if methodpccoarse==4
			...
			function compPC4b(iB,H,R)
				...
				return x -> diagshift*x+scalef2*(PC1a*(PC1b*(HI*(PC2*(HI'*(PC1b*(PC1a*x)))))))
...
#Now B2D on fine resolution model !
			...
			function compPC5(iB,H,R)
				#return x -> diagshift*x-diagshift*(HI*(HI'*x))+scalef2*(HI*(PC1*(PC2*(PC1*(HI'*x)))))
				return x -> diagshift*x+scalef2*(PC1a*(PC1b*(HI*(PC1*(PC2*(PC1*(HI'*(PC1b*(PC1a*x)))))))))
			end
