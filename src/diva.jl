...
function diva3d(
    ...
    if surfextend
        ...
        len = ntuple(i -> if typeof(len[i]) <: Array
        ...
        plotres_ext(timeindex, sel, fit, erri) =
            plotres(timeindex, sel, fit[:, :, 2:end], erri[:, :, 2:end])
    end
    ...
    Dataset(filename, "c") do ds
        ...
        for timeindex = 1:length(TS)
            ...
            for i = 1:niter_e
                ...
                errortype, kwargs2 = if i == niter_e
                    :cpme, kwargs
                else
