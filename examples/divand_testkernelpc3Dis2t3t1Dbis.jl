...
function compPC(iB,H,R)
    ...
    function fun!(x,fx)
        fx[:] = diagshift*x+scalef2*(PC1*(PC2*(PC3*((PC2*(PC1*x))))))
    end
    return fun!
...
@time fiiter,s = divandrun(mask,(pm,pn,po),(xi,yi,zi),(x,y,z),f,len,epsilon2;alphabc=1,pcargs...,
                           inversion=:pcg,compPC = compPC, fi0 = unpack(sv,scalef2*xgs2)[1],btrunc=2);
