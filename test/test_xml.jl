...
function loadoriginators4(csvfile::IO)

    header = readline(csvfile)

    @show strip.(split(header,","),'"')
    @assert strip.(split(header,","),'"') == ["active", "author_edmo",
                                              "cdi_identifier", "originator_edmo"]
    ...
    @show "here"
    while !eof(csvfile)
        line = strip(readline(csvfile))
        #@show line
        active, author_edmo, cdi_identifier,originator_edmos = strip.(split(line,","),'"')

        key = (parse(Int,author_edmo), cdi_identifier)
        db[key] = (lowercase(active) == "true",
                   parse.(Int64,split(originator_edmos,',')))
    end
...
function get_originators_from_obsid(db,filepath,errname; ignore_errors = false)
...
function loadNC(filepath,varname,project;
                cdilist = "/home/abarth/workspace/divadoxml-gui/CDI-list-export.zip",
                ignore_errors = false)
...
domain = strip(templateVars["domain"])
edmo_code = templateVars["edmo_code"]
varname = templateVars["varname"]
doi = get(templateVars,"doi","")
...
originators = get_originators_from_obsid(    
    db,filepath,errname,
...
bbox = join([string(templateVars[c]) for c in ["longitude_min","latitude_min","longitude_max","latitude_max"]],',')
#print("entry",entry,bbox,templateVars["default_field_max"])
...
merge!(templateVars, Dict(
    ...
    #                 "description" => "WMS layer for " * templateVars["title"]}],
    "WMS_layers"  => [],
...
cdilist = "/home/abarth/workspace/divadoxml-gui/CDI-list-export.zip"
cdilist = "/home/abarth/workspace/divadoxml-gui/export.csv"

templateVars = loadNC(filepath,varname,project,
                     cdilist = cdilist,
                     ignore_errors = ignore_errors)
