...

{
    ...
    "category": "function",
    "text": "obsvalue,obslon,obslat,obsdepth,obstime,obsids = NCODV.load(T,fname,long_name;\n     qv_flags = [\"good_value\",\"probably_good_value\"])\n\nLoad all profiles in the file fname corresponding to netcdf variable with the long_name attribute equal to the parameter long_name. qv_flags is a list of strings with the quality flags to be kept. obsids is a vector of strings with the EDMO code and local CDI id concatenated by a hyphen.\n\n\n\n\n\n"
},
...

{
    ...
    "category": "function",
    "text": "sucess = add!(qt,x,attrib,max_cap = 10) Add point x with the attribute attrib to the quadtree qt. sucess is true if x is within the bounds of the quadtree node qt (otherwise false and the point has not been added)\n\n\n\n\n\n"
},
...

{
    ...
    "category": "function",
    "text": "Test if the rectangles defined by x0,x1 and y0,y1 intersect              x1   +–––––+   |          |   |   +–––––+ y1   |   |      |   |   +–––––+   |  x0   |          |       |          |       +–––––+      y0\n\n\n\n\n\n"
},
...

{
    ...
    "category": "function",
    "text": "Derive fractional indices on a separable grid.\n\nI = localizeseparablegrid(xi,mask,x)\n\nxi is a tuple of vectors and x and tuple of n-dimensional arrays, e.g.\n\nx1,x2 = ndgrid(2 * collect(1:5),collect(1:6)) x = (x1,x2)\n\nDerive fractional indices where xi are the points (typical discrete observations) to localize in the separable grid x (every dimension in independent on other dimension). The output I is an n-by-m array where n number of dimensions and m number of observations. The correspond element of I is negative if xi is outside of the grid defined by x.\n\n\n\n\n\n"
},
...
{
    "location": "#Correlation-length-1",
    "page": "DIVAnd.jl documentation",
    "title": "Correlation length",
    "category": "section",
    "text": "The estimation of the correlation length in the function diva3d can be activated with the option fitcorrlen for the horizontal and vertical correlaton. The parameter len should then an empty tuple (()) or a tuple of arrays equal to one. The actually used correlation length is a product between the prodived values of the array len and the estimated correlation length by fitting. Setting fitcorrlen to true means thus that the intepretation of the parameters changes from absolution correlation length to relative correlation length.The estimation of the horizontal and vertical correlation can also be activated selectively by just setting fithorzcorrlen and fitvertcorrlen (respectively) to true.If one wants to not use the vertical correlation length, the one can put the corresponding value in len to zero. Consequently the value of fitvertcorrlen and fitcorrlen should be keep to false (i.e. its default values). Optimizing the horizontal correlation length is still possible by setting fithorzcorrlen to true."
},
...
{
    "location": "#My-parameter-represent-a-concentration-and-I-get-unrealistic-negative-values-1",
    "page": "DIVAnd.jl documentation",
    "title": "My parameter represent a concentration and I get unrealistic negative values",
    "category": "section",
    "text": "You can use an anamorphosis transform, in particular DIVAnd.Anam.loglin. The idea is that the transformed variable is close to a Gaussian distribution that the original variable.\nUse the option fieldmin = 0.0 of diva3dIf the parameter epsilon of DIVAnd.Anam.loglin is larger than zero (which is necessary if some measurements are exactly zero), then the smallest value that analysis can have is -epsilon. Therefore the option fieldmin is still required to avoid negative values."
},
...

{
    ...
    "category": "section",
    "text": "To update the documentation locally, install the package Documenter and run the script include(\"docs/make.jl\").using Pkg\nPkg.add(\"Documenter\")"
},
...
{
    "location": "#Installation-problem-of-PyPlot-on-Linux-(Debian/Ubuntu)-1",
    "page": "DIVAnd.jl documentation",
    "title": "Installation problem of PyPlot on Linux (Debian/Ubuntu)",
    "category": "section",
    "text": "Make sure that the following Debian/Ubuntu packages are installed:sudo apt-get install python3 libpython3 python3-tkThen start Julia and run:using Pkg\nPkg.build(\"PyCall\")\nPkg.build(\"PyPlot\")Test PyPlot with:using PyPlot\nplot(1:10)"
},
...

{
    ...
    "category": "section",
    "text": "The zlib library of RedHat 6, is slightly older than the library which EzXML.jl and libxml2 requires.To verify this issue, you can type in Juliausing Libdl\nusing Pkg\nLibdl.dlopen(joinpath(Pkg.dir(\"EzXML\"),\"deps/usr/lib/libxml2.so\"))It should not return an error message. On Redhat 6.6, the following error message is returned:ERROR: could not load library \"/home/username/.../EzXML/deps/usr/lib/libxml2.so\"\n\n/lib64/libz.so.1: version `ZLIB_1.2.3.3\' not found (required by /home/.../EzXML/deps/usr/lib/libxml2.so)\n\nStacktrace:\n\n [1] dlopen(::String, ::UInt32) at ./libdl.jl:97 (repeats 2 times)A newer version zlib can be installed by the following command:using Pkg\nPkg.add(\"CodecZlib\")However, the following command should work: LD_LIBRARY_PATH=\"$HOME/.julia/full/path/to/CodecZlib/.../deps/usr/lib/:$LD_LIBRARY_PATH\" julia --eval  \'print(Libdl.dlopen(joinpath(Pkg.dir(\"EzXML\"),\"deps/usr/lib/libxml2.so\"))\'by replacing the file path appropriately. (see also https://github.com/JuliaLang/julia/issues/7004, https://github.com/JuliaIO/HDF5.jl/issues/97, and https://github.com/bicycle1885/EzXML.jl/issues/102)To make Julia use this library, a user on RedHat 6 should always start Julia with:LD_LIBRARY_PATH=\"$HOME/.julia/full/path/to/CodecZlib/.../deps/usr/lib/:$LD_LIBRARY_PATH\" juliaOne can also create script with the following content:#!/bin/bash\nexport LD_LIBRARY_PATH=\"$HOME/.julia/full/path/to/CodecZlib/.../deps/usr/lib/:$LD_LIBRARY_PATH\"\nexec /path/to/bin/julia \"$@\"by replacing /path/to/bin/julia to the full path of your installation directory. The script should be marked executable and it can be included in your Linux search PATH environement variable. Julia can then be started by calling directly this script."
},

{
    ...
    "category": "section",
    "text": "Running using Pkg; Pkg.test(\"DIVAnd\") fails with the error:automatic download failed (error: 2147500036)The test suite will download some sample data. You need to have internet access and run the test function from a directory with write access.You can change the directory to your home directory with the julia command cd(homedir()).You can check the current working directory with:pwd()"
},

{
    "location": "#METADATA-cannot-be-updated-in-Julia-0.6-1",
    "page": "DIVAnd.jl documentation",
    "title": "METADATA cannot be updated in Julia 0.6",
    "category": "section",
...
{
    "location": "#Kernel-not-working-with-IJulia/Jupyter-under-Julia-0.7-Windows-1",
    "page": "DIVAnd.jl documentation",
